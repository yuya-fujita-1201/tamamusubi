...
PROC = {
    ...
    "tile.wa_stairs":     dict(mode="tile",            fw=128, fh=128, cols=1, rows=1, out="tile_wa_stairs.png",    keys=["tile.wa_stairs"]),
    "tile.wa_cliff":      dict(mode="tileset",         fw=128, fh=128, cols=4, rows=4, out="tile_wa_cliff.png",     keys=["tile.wa_cliff_set"]),
    "tile.wa_ishigaki":   dict(mode="tileset",         fw=128, fh=128, cols=4, rows=4, out="tile_wa_ishigaki.png",  keys=["tile.wa_ishigaki_set"]),
    ...
    # ── Phase L3 GPT-momo2三原則・棚田高低差フルセット（曲線オートタイル/草付き土手/丸角水田/川岸/森縁/装飾overlay）──
    "tile.ka_grass_calm": dict(mode="tileset", fw=128, fh=128, cols=4, rows=4, out="tile_ka_grass_calm.png", keys=["tile.ka_grass_calm_set"]),
    "tile.ka_dote":       dict(mode="tileset",         fw=128, fh=128, cols=4, rows=4, out="tile_ka_dote.png",     keys=["tile.ka_dote_set"]),
    "tile.ka_ishigaki":   dict(mode="tileset",         fw=128, fh=128, cols=4, rows=4, out="tile_ka_ishigaki.png", keys=["tile.ka_ishigaki_set"]),
    "tile.ka_paddy2":     dict(mode="tileset",         fw=128, fh=128, cols=4, rows=4, out="tile_ka_paddy2.png",   keys=["tile.ka_paddy2_set"]),
    "tile.ka_river2":     dict(mode="tileset",         fw=128, fh=128, cols=4, rows=4, out="tile_ka_river2.png",   keys=["tile.ka_river2_set"]),
    "tile.ka_grass2":     dict(mode="tileset",         fw=128, fh=128, cols=4, rows=4, out="tile_ka_grass2.png",   keys=["tile.ka_grass2_set"]),
    "tile.ka_path2":      dict(mode="tileset",         fw=128, fh=128, cols=4, rows=4, out="tile_ka_path2.png",    keys=["tile.ka_path2_set"]),
    "tile.ka_forest2":    dict(mode="tileset",         fw=128, fh=128, cols=4, rows=4, out="tile_ka_forest2.png",  keys=["tile.ka_forest2_set"]),
    "tile.ka_bamboo":     dict(mode="tileset",         fw=128, fh=128, cols=4, rows=4, out="tile_ka_bamboo.png",   keys=["tile.ka_bamboo_set"]),
    "tile.ka_decor":      dict(mode="tileset_overlay", fw=128, fh=128, cols=4, rows=4, out="tile_ka_decor.png",    keys=["tile.ka_decor_set"]),
    "obj.waterfall":      dict(mode="prop", fw=128, fh=128, cols=1, rows=1, out="obj_waterfall.png",      keys=["obj.waterfall"]),
    "obj.jizo":           dict(mode="prop", fw=128, fh=128, cols=1, rows=1, out="obj_jizo.png",           keys=["obj.jizo"]),
    "obj.flower_cluster": dict(mode="prop", fw=128, fh=128, cols=4, rows=1, out="obj_flower_cluster.png", keys=["obj.flower_cluster"]),
    "obj.bridge":         dict(mode="prop", fw=128, fh=128, cols=1, rows=1, out="obj_bridge.png",         keys=["obj.bridge"]),
