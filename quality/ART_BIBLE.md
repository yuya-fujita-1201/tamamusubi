...
3. **ブロック下部の影**: 段差ブロック**そのセル**の deco に `waBaseShadow`（下が黒い半透明グラデ）を敷き、ブロックの足元を陰にする

> ★**影ルール（2026-06-17 ユーザー改定・厳守）**:
> - 黒のグラデで高低差を出してよいのは **石垣・階段・滝など「高低差をつけるためのブロック」だけ**。
> - 影は **ブロックの下部**に入れる（旧版のように真下の草/道/水セルへ落とさない＝あれは違和感の塊）。
> - **川の水・草原・道など非・段差物には黒グラデを一切落とさない**（川岸影・水際影は廃止）。
> - 実装: `tanada.ts` 末尾で ISHI/SHRINE_WALL/DOTE/WFALL のセル自身に `waBaseShadow` を重ねる。`waBaseShadow` = `waDropshadow` の上下反転（下黒）。
...
* 川岸に黒グラデの接地影は**敷かない**（2026-06-17 改定＝非段差物への黒グラデ禁止）。低地感はカットバンク岸の有機的な曲線と岸タイルで出す
...
* 各段の南端に `kaIshigaki`(FACE=5) を配置し、その石垣セル自身の下部に `waBaseShadow` を敷く（真下の草/水には落とさない）
...
* **棚田**: 段々になった水鏡が見え、南面に石垣があり、石垣ブロックの下部が陰になり、滝が石垣の切り欠きから落ちて滝壺に吸い込まれる。水田と草地の境界が有機的な曲線になっている
* **川**: 周囲の草地より明らかに低い位置を流れているように見え（岸の曲線で表現・黒グラデ影は使わない）、橋の下を川が通っている構造が読める
